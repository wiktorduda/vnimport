...
class UpdateGameModel:
    ...
    def update_name(self, game_obj, model):
        if self.is_roman:
            ...
            self.update_original_name(game_obj, model)

    def update_original_name(self, game_obj, model):
        name = game_obj['original_name']
        model.Name = name
    ...
    def update_links(self, game_obj, model):
        ...
        playnite_links = []
        for link in links:
            playnite_link = self.playnite.SDK.Models.Link(link['name'], link['url'])
            playnite_links.append(playnite_link)
        model.Links = System.Collections.ObjectModel.ObservableCollection[self.playnite.SDK.Models.Link](playnite_links)
